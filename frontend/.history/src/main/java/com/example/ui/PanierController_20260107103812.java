...
public class PanierController {
    ...
    private void onConfirmer() {
        // ✅ vers page confirmation (celle avec numéro de commande)
        goTo("ui/confirmation/confirmation.fxml");
    }

    @FXML
private void onConfirmerCommande() {

    Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
    alert.setTitle("Confirmation de commande");
    alert.setHeaderText("Êtes-vous sûr de vouloir confirmer ?");
    ...
    alert.getButtonTypes().setAll(btnAnnuler, btnContinuer);

    Optional<ButtonType> result = alert.showAndWait();

    if (result.isPresent() && result.get() == btnContinuer) {
        // ✅ L’utilisateur confirme vraiment
        confirmerCommande();
    }
}
